...
SEX_CRIME = ["rape","sexual assault","sex abuse","sexual abuse","child sexual abuse","molestation","molested","child pornography","csam","exploitation","grooming","solicitation","solicitation of a minor","minor sex","abuse"]
LEGAL_STATUS = ["arrested","charged","charged with","convicted","sentenced","guilty plea","pleaded guilty","indicted"]
NOT_A_TRANS_SIGNALS = [
    "republican","gop","maga","conservative activist","anti-lgbt","anti-trans",
    "parents' rights","parents rights","family values","christian school official",
    "christian school head","youth pastor","church leader","public moral crusader",
    "values campaigner","anti-gay","anti-drag"
]
...
NOT_A_TRANS_NEGATIVE = ["illegal immigrant","undocumented immigrant","immigration status","visa fraud","border crossing","asylum fraud","operating as pastor"]
...
def evaluate_article(article):
    ...
    gc = collect_matches(GENERAL_CRIME, blob)
    sx = collect_matches(SEX_CRIME, blob)
    ls = collect_matches(LEGAL_STATUS, blob)
    nas = collect_matches(NOT_A_TRANS_SIGNALS, blob)
    nat_neg = collect_matches(NOT_A_TRANS_NEGATIVE, blob)
    lx = collect_matches(LAW_ORDER_ACTORS, blob)
    le = collect_matches(LAW_ORDER_EVENTS, blob)
    s = collect_matches(SCANDAL, blob)

    special = bool(sx and ls and nas)

    if nat_neg and not special:
        return {
            "bucket":"reject",
            "score":0.0,
            "tags":[],
            ...
            "reason":"Rejected: immigration/status or generic church-title story without conservative-hypocrisy relevance."
        }
    ...
    score = max(0.0, min(10.0, round(len(i)*1.8 + len(o)*1.0 + len(a)*0.8 + len(nas)*1.8 + len(lx)*1.0 + len(le)*1.0 - len(s)*2.2, 1)))
    ...
    if special:
        return {
            ...
            "angle":"not a trans...",
            "summary":article.get("summary","")[:500],
            "reason":"Sex-crime legal story involving an explicit conservative/culture-war/moral-authority hypocrisy signal."
        }
    if law:
        return {
            "bucket":"law_and_order",
            "score":score,
            "tags":["law-and-order"],
            "angle":"law & order",
            "summary":article.get("summary","")[:500],
            "reason":"Documented legal action against a Republican government body, federal agency, or Republican office holder."
        }
    if violent_non_special or (gc and not i):
        return {
            ...
            "angle":"off-theme",
            "summary":article.get("summary","")[:500],
            "reason":"Generic crime or violent legal story outside the site's scope."
        }
    if s and not i:
        return {
            "bucket":"reject",
            ...
            "angle":"off-theme",
            "summary":article.get("summary","")[:500],
            "reason":"Generic scandal or corruption story."
        }
    if i and (o or a or score >= 4.5):
        return {
            ...
            "score":score,
            "tags":[],
            "angle":"identity-outrage story",
            "summary":article.get("summary","")[:500],
            "reason":"On-theme identity/pluralism backlash story."
        }
    if i or (a and o):
        return {
            "bucket":"wings",
            ...
            "angle":"identity-outrage story",
            "summary":article.get("summary","")[:500],
            "reason":"Borderline but worth a second look."
        }
    return {
        "bucket":"reject",
        "score":score,
        ...
        "angle":"off-theme",
        "summary":article.get("summary","")[:500],
        "reason":"Off-theme politics or generic news."
    }
