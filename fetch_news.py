...
MAX_CANDIDATES_PER_FEED=int(os.getenv("MAX_CANDIDATES_PER_FEED","100")); MAX_AI_REVIEWS_PER_RUN=int(os.getenv("MAX_AI_REVIEWS_PER_RUN","500"))
...
IGNORE_SEEN=os.getenv("IGNORE_SEEN","0").lower() in {"1","true","yes","y"}; ROLLING_DAYS=int(os.getenv("ROLLING_DAYS","30"))
NOT_A_TRANS_START=os.getenv("NOT_A_TRANS_START","2025-01-20"); LAW_AND_ORDER_START=os.getenv("LAW_AND_ORDER_START","2025-01-20")
TAGLINE = "A satirical chronicle of fear, grievance, and cultural alarm in the American white-as-2%-milk echo chamber."
WEATHER_FILE = "images/weather_super_snowflakey.png"
WEATHER_ALT = "NAWWP WEATHER ALERT: It's Super Snowflakey Out There! Fragility advisory remains in effect coast to coast."
...
def build_google_news_rss(query,special=False,law=False):
    q=urllib.parse.quote(query)
    if special: return f"https://news.google.com/rss/search?q={q}+after:{NOT_A_TRANS_START}&hl=en-US&gl=US&ceid=US:en"
    if law: return f"https://news.google.com/rss/search?q={q}+after:{LAW_AND_ORDER_START}&hl=en-US&gl=US&ceid=US:en"
    return f"https://news.google.com/rss/search?q={q}+when:{ROLLING_DAYS}d&hl=en-US&gl=US&ceid=US:en"
...
def analyze_text(text):
    ...
    score=round(len(i)*1.8+len(o)*1.0+len(a)*0.8+len(la)*1.0+len(le)*1.0-len(gc)*2.5-len(s)*2.2,1)
    ...
    maybe_law=(len(la)>=1 and len(le)>=1) or (("ice" in t or "dhs" in t or "federal agency" in t or "trump administration" in t) and len(le)>=1)
    return {"maybe_relevant":maybe_general,"maybe_special":bool(sc and sa),"maybe_law":maybe_law,"lexical_score":score}
...
def load_feed_specs():
    ...
    for item in cfg.get("rss_feeds",[]):
        if item.get("enabled",True): out.append({"label":item["label"],"url":item["url"],"state":item.get("state"),"special":False,"law":False})
    for item in cfg.get("google_news_queries",[]):
        if item.get("enabled",True): out.append({"label":item["label"],"url":build_google_news_rss(item["query"],False,False),"state":item.get("state"),"special":False,"law":False})
    for item in cfg.get("google_news_queries_special",[]):
        label=item["label"]
        if item.get("enabled",True):
            is_law="Law & Order" in label
            out.append({"label":label,"url":build_google_news_rss(item["query"],False,is_law),"state":item.get("state"),"special":(not is_law),"law":is_law})
    return out
def fetch_candidates(spec):
    ...
    for entry in entries:
        ...
        if spec.get("special"):
            ...
            if not analysis["maybe_special"]: continue
        elif spec.get("law"):
            if not is_after_fixed_start(published,LAW_AND_ORDER_START): continue
            if not analysis["maybe_law"]: continue
        else:
            if not is_within_days(published,ROLLING_DAYS): continue
            if not (analysis["maybe_relevant"] or analysis["maybe_law"]): continue
        out.append({"title":title,"url":url,"summary":summary[:1000],"published":published,"source":spec["label"],"state":spec.get("state"),"special_query":spec.get("special",False),"law_query":spec.get("law",False),"prefilter":analysis})
    return out
def format_date(s):
...
def weather_banner():
    return f'<section class="weather-banner"><div class="weather-banner-inner"><img src="{WEATHER_FILE}" alt="{WEATHER_ALT}"></div></section>'
...
def section_name(item):
    ...
    blob=" ".join([item.get("title",""),item.get("summary",""),(item.get("angle") or "")]).lower()
    if any(x in blob for x in ["school","curriculum","school board","library","voucher"]): return "Education & Schools"
    if any(x in blob for x in ["transgender","gender ideology","pronoun","drag","pride"]): return "Gender & Sexuality"
    if any(x in blob for x in ["muslim","islamic school","religious liberty","christian values","traditional values"]): return "Religion & Pluralism"
    if any(x in blob for x in ["dei","diversity","equity","white people","white boys","immigrant","immigration","refugee"]): return "Race, DEI & Immigration"
    return "Top Stories"
def build_sections(kept,law_and_order,not_a_trans):
    ...
    return {"lead":lead,"lead_side":lead_side,"top_stories":top_stories,"education":sections["Education & Schools"][:16],"gender":sections["Gender & Sexuality"][:16],"religion":sections["Religion & Pluralism"][:16],"race":sections["Race, DEI & Immigration"][:16],"law_and_order":law_and_order[:80],"not_a_trans":not_a_trans[:60]}
...
def main():
    ...
    for item in raw:
        key=article_key(item); title_key=re.sub(r"\W+"," ",item.get("title","").lower()).strip()
        if (not IGNORE_SEEN) and (item.get("special_query") or item.get("law_query")) and key in seen: continue
        if (not IGNORE_SEEN) and (not item.get("special_query")) and (not item.get("law_query")) and (key in seen or title_key in seen): continue
    ...
    reviews.extend(review_rows); reviews=reviews[-18000:]
    ...
    archived_law=[r for r in reviews if r.get("bucket")=="law_and_order" and is_after_fixed_start(r.get("published",""),LAW_AND_ORDER_START)]
