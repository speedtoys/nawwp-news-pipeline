...
MAX_CANDIDATES_PER_FEED = int(os.getenv("MAX_CANDIDATES_PER_FEED", "100"))
MAX_AI_REVIEWS_PER_RUN = int(os.getenv("MAX_AI_REVIEWS_PER_RUN", "360"))
KEEP_MIN_SCORE = float(os.getenv("KEEP_MIN_SCORE", "4.0"))
WINGS_MIN_SCORE = float(os.getenv("WINGS_MIN_SCORE", "2.5"))
FETCH_WORKERS = int(os.getenv("FETCH_WORKERS", "12"))
...
NOT_A_TRANS_START = os.getenv("NOT_A_TRANS_START", "2025-01-20")
...
SECTION_PAGES = [("index.html","Front Page"),("education.html","Education & Schools"),("gender.html","Gender & Sexuality"),("religion.html","Religion & Pluralism"),("race.html","Race, DEI & Immigration"),("not-a-trans.html","Not A Trans…"),("wings.html","In the Wings")]
...
def parse_iso(iso_str):
    try: return dt.datetime.fromisoformat(iso_str.replace("Z", "+00:00"))
    except Exception: return None

def is_within_days(iso_str, days):
    d = parse_iso(iso_str)
    return bool(d and d >= (dt.datetime.now(dt.timezone.utc) - dt.timedelta(days=days)))

def is_after_fixed_start(iso_str, start_date):
    d = parse_iso(iso_str)
    return bool(d and d >= dt.datetime.fromisoformat(start_date + "T00:00:00+00:00"))
...
def analyze_text(text):
    t = (text or "").lower()
    i = collect_matches(IDENTITY, t); o = collect_matches(OUTRAGE, t); a = collect_matches(ACTORS, t)
    gc = collect_matches(GENERAL_CRIME, t); sc = collect_matches(SPECIAL_CRIME, t); sa = collect_matches(SPECIAL_ACTORS, t); s = collect_matches(SCANDAL, t)
    score = round(len(i)*1.8 + len(o)*1.0 + len(a)*0.8 - len(gc)*2.5 - len(s)*2.6, 1)
    ...
    maybe_special = bool(sc and sa)
    return {"maybe_relevant": maybe_general, "maybe_special": maybe_special, "lexical_score": score}
...
def fetch_candidates(spec):
    ...
    for entry in entries:
        ...
        if spec.get("special"):
            ...
            if not analysis["maybe_relevant"]: continue
...
def section_name(item):
    ...
    if {"book-bans","parents-rights"} & tags or any(x in blob for x in ["school","curriculum","school board","library","voucher"]): return "Education & Schools"
    if {"anti-trans","lgbtq-panic"} & tags or any(x in blob for x in ["transgender","gender ideology","pronoun","drag","pride"]): return "Gender & Sexuality"
    if {"anti-muslim","religious-liberty"} & tags or any(x in blob for x in ["muslim","islamic school","religious liberty","christian values","traditional values"]): return "Religion & Pluralism"
    if {"anti-dei","white-grievance","immigration"} & tags or any(x in blob for x in ["dei","diversity","equity","white people","white boys","immigrant","immigration","refugee"]): return "Race, DEI & Immigration"
    return "Top Stories"

def build_sections(kept, not_a_trans):
    ...
    return {"lead": lead, "lead_side": lead_side, "top_stories": top_stories, "education": sections["Education & Schools"][:16], "gender": sections["Gender & Sexuality"][:16], "religion": sections["Religion & Pluralism"][:16], "race": sections["Race, DEI & Immigration"][:16], "not_a_trans": not_a_trans[:40]}
...
def build_page_html(payload):
    ...
    return {
        ...
        "not-a-trans.html": page_shell("Not A Trans…", "not-a-trans.html", render_section("Not A Trans…", "Arrests, charges, convictions, and sentencing stories involving explicitly conservative, MAGA, or religious figures in sex-crime cases. This section stays separate and searches back to 2025-01-20.", parts["not_a_trans"], compact=True), payload),
        "wings.html": page_shell("In the Wings", "wings.html", render_section("In the Wings", "Borderline or adjacent stories from the last 30 days.", wings[:40], compact=True), payload),
    }
...
def main():
    ...
    reviewed, current_kept, current_wings, current_rejected, current_special = [], [], [], [], []
    with ThreadPoolExecutor(max_workers=AI_WORKERS) as ex:
        futures = {ex.submit(evaluate_article, item): item for item in candidates}
        for fut in as_completed(futures):
            item = futures[fut]; row = {**item, **fut.result()}; reviewed.append(row)
    ...
    reviews.extend(review_rows); reviews = reviews[-12000:]
    ...
    payload = {
        ...
        "counts": {"kept": len(merged_kept), "wings": len(merged_wings), "not_a_trans": len(merged_special), "rejected": len(current_rejected), "reviewed": len(reviewed), "new_kept_this_run": len(current_kept), "new_wings_this_run": len(current_wings), "new_not_a_trans_this_run": len(current_special), "rolling_days": ROLLING_DAYS, "not_a_trans_start": NOT_A_TRANS_START},
        "kept": merged_kept, "in_the_wings": merged_wings, "not_a_trans": merged_special, "rejected": sorted(current_rejected, key=lambda x: x.get("score", 0), reverse=True)[:200]
    }
    ...
    print(f"Published windows: kept={len(merged_kept)} wings={len(merged_wings)} not_a_trans={len(merged_special)}")
    print(f"General window days={ROLLING_DAYS} ; Not A Trans start={NOT_A_TRANS_START}")
